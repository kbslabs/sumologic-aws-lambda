...
sam deploy --no-execute-changeset --capabilities CAPABILITY_IAM --stack-name serverlessrepo-sumologic-loggroup-connector --template packaged.yaml --region $AWS_REGION --parameter-overrides LogGroupPattern=$LOG_GROUP_PATTERN LambdaARN=$LAMBDA_ARN DestinationArnValue="arn:aws:lambda:us-west-2:529513974030:function:serverlessrepo-sumologic-SumoLogGroupLambdaConnec-1MMCQVW6C6OEO"
